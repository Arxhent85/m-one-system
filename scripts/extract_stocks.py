        ...
        for row in root.findall('.//{http://schemas.openxmlformats.org/spreadsheetml/2006/main}row'):
            ...
            if len(vals) >= 2:
                ...
                if sku.isdigit() and len(sku) >= 4 and name and 'EM' not in name and 'SHIFRA' not in sku:
                    # Find stock quantity from remaining cells
                    stock_qty = 100
                    for cell_val in vals[2:]:
                        try:
                            num = int(float(str(cell_val).strip()))
                            if num > 0:
                                stock_qty = num
                                break
                        except:
                            pass
                    stocks[sku] = (name, stock_qty)

    print("export const ALL_PRODUCTS = [")
    for sku in sorted(stocks.keys()):
        name, qty = stocks[sku]
        clean_name = name.replace("'", "\\'")
        print(f"  {{ id: 'p-{sku}', sku: '{sku}', name: '{clean_name}', stock: {qty} }},")
    print("]")
