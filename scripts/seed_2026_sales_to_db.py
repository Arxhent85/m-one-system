...
print("=== PARSING 2026 SALES DATA AND REBUILDING COMPLETE SEED SQL ===")
...
for row in sheet_cust.iter_rows(min_row=2, values_only=True):
    ...
    c_nr_str = str(int(k_nr)) if isinstance(k_nr, (int, float)) else (str(k_nr).strip() if k_nr else "10000")
    c_name = str(k_name).replace("'", "''").strip()
    ort_str = str(ort).replace("'", "''").strip() if ort else ''
    tel_str = str(tel).replace("'", "''").strip() if tel else ''
    
    if c_nr_str.startswith('1'): agent_assigned = 'Qerimi'
    elif c_nr_str.startswith('2'): agent_assigned = 'Mensuri'
    elif c_nr_str.startswith('3'): agent_assigned = 'Miloti'
    else: agent_assigned = str(axhenti).replace("'", "''").strip() if axhenti else 'M ONE'
...
# 4. VERKÄUFE 2026 PARSEN (Daten 2026.xlsx)
wb_sales = openpyxl.load_workbook(os.path.join(folder, 'Daten 2026.xlsx'), data_only=True, read_only=True)
s_sales = wb_sales['Sheet1']
order_rows = []
order_count = 0

for i, row in enumerate(s_sales.iter_rows(min_row=5, values_only=True)):
    if not any(row): continue
    ...
    try: val = round(float(wert), 2) if wert is not None else 0.0
    except: val = 0.0
    
    date_str = str(datum)[:10] if datum else '2026-01-07'
    agent_str = str(agent).strip() if agent else 'Qerimi'
    
    # Location based on agent
    if 'Mensuri' in agent_str:
        loc_id = '22222222-2222-2222-2222-222222222222'
    elif 'Qerimi' in agent_str:
        loc_id = '33333333-3333-3333-3333-333333333333'
    else:
        loc_id = '11111111-1111-1111-1111-111111111111'
        
    faktura_no = str(faktura).replace("'", "''").strip() if faktura else f"FK-2026-{i:04d}"
    order_count += 1
    
    order_rows.append(f"('{faktura_no}', (SELECT id FROM customers WHERE company_name='{c_name}' LIMIT 1), '{loc_id}', 'confirmed', 'invoice', 'paid', {val}, {val}, '{date_str}T10:00:00Z')")

wb_sales.close()
print(f"   - {order_count} echte Verkaufsaufträge aus Daten 2026.xlsx geladen")

# 5. COMPLETE SEED SQL GENERIEREN
lines = [
    "-- ============================================================",
...
# Verkäufe 2026
lines.append("-- 5. ECHTE VERKÄUFE 2026 (3.997 RECHNUNGEN)")
for b in range(0, len(order_rows), 200):
    batch = order_rows[b:b+200]
    lines.append("INSERT INTO sales_orders (order_number, customer_id, location_id, status, payment_method, payment_status, subtotal, total_amount, created_at) VALUES")
    lines.append(",\n".join(batch) + " ON CONFLICT DO NOTHING;\n")

with open(sql_out, 'w', encoding='utf-8') as f:
    f.write("\n".join(lines))

print(f"SUCCESSFULLY GENERATED seed_real_data.sql WITH ALL {order_count} REAL 2026 SALES!")
